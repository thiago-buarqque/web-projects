...
interface props {
    ...
    isOtherPlayerWaitingForRematch: boolean;
    otherPlayerName: string;
    isUserPlayer1: boolean;
}

const MultiPlayerGameEndPopUp: React.FC<props> = ({
    gameDifficultyLevel,
        ...
        <div id="game_finish" className={`${!isGameFinished ? `__hide_game_end_screen` : `__show_game_end_screen`}`}>
            <div id="__content">
                <div id="__congrats">
                    ...
                    <span id="__congrats_msg">{`${isFinishedGameValid ? `Você` : winnerName} venceu.`}</span>
                    <span id="__match_time">{`Tempo: ${matchTime}`}</span>
                </div>
                <div id="__options">
                    <button className="__menu_option main_option" onClick={newGame}>
                        <span>{isWaitingForRematch ? `Esperando por revanche` : `Revanche`}</span>
                    </button>
                    {
                        isOtherPlayerWaitingForRematch ? <span id='rematch_flag'>{`${otherPlayerName} solicita revanche.`}</span> : ``
                    }
                    <span></span>
                    {isUserPlayer1 ? (
                        <DifficultyChanger
...
export default MultiPlayerGameEndPopUp;
